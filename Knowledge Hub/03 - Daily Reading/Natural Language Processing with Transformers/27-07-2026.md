...
- Trong encoder-decoder models như T5/BART, decoder vừa dùng causal self-attention để nhìn output đã sinh, vừa dùng [[Cross-Attention|cross-attention]] để nhìn representation từ encoder.
- [[Attention Mask|Mask matrix]] là cách biểu diễn luật "token nào được nhìn token nào" trong attention. Với causal mask, ma trận thường có dạng tam giác dưới: token hiện tại được nhìn chính nó và quá khứ, nhưng không được nhìn tương lai.
...
| Nhóm | Cách nhìn context | Cách dùng | Ví dụ |
| --- | --- | --- | --- |
| Encoder-only | Nhìn toàn bộ input hai chiều | Hiểu/biểu diễn input | BERT, RoBERTa |
| Decoder-only | Chỉ nhìn prefix/quá khứ | Sinh tiếp văn bản | GPT |
| Encoder-decoder | Encoder đọc input, decoder sinh output | Biến đổi sequence | T5, BART |
...
- Với phần **Meet the Transformers**, mình không nên nhớ từng model rời rạc. Nên nhớ theo "cấu trúc -> cơ chế attention -> task":
  ...
  - T5/BART/M2M-100 family: encoder-decoder -> encoder đọc, decoder viết -> seq2seq.
- Mental model: tên model là ví dụ; thứ cần nhớ là family của nó quyết định kiểu context mà model được nhìn và loại task nó làm tốt.
...
- Padding mask và causal mask khác nhau thế nào trong một batch có sequence dài ngắn khác nhau?
- M2M-100 khác T5/BART ở phạm vi multilingual translation như thế nào?
- Cross-attention weights có thể dùng để hiểu decoder đang chú ý phần nào của input không?
...
- [x] Ghi chú mask matrix / [[Attention Mask]]
- [x] Tạo bảng so sánh encoder/decoder/encoder-decoder
- [x] Ghi chú phần Meet the Transformers
- [x] Tạo và link concept [[Cross-Attention]]
